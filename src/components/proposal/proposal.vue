<template>
	<div id="proposal">
		<div class="mui-scroll-wrapper">
			<div class="mui-scroll" id="scrollPro">
				<div>
					<!--遍历数据-->
					<ul class="prop-list">
						<li v-for="(item, index) in itemData">
							<div @tap="goDetail(item, item.suggest.id)">
							...
							<div class="prop-content">
								<div class="img-list mui-clearfix" v-show="item.imgList.length === 0 ? false : true">
									<div class="img-box" v-for="(img, index) in item.imgList" >
										<img v-show="img.img_path" :src="img_URL + img.img_path" @tap="previewImg(item.imgList, index)">
									</div>
								</div>
							</div>
							<div class="operation mui-clearfix">
								<div class="box">
									<i class="iconfont" :class="item.suggest.is_digg === '0' ? 'icon-xiangqing_dianzan' : 'icon-xiangqing_yidianzan-1'" @tap="fabulous(item.suggest, index)"><span v-text="item.diggList.length"></span></i>
									<i class="iconfont icon-xiangqing_pinglun" @tap="showPop(item.suggest.id, index)"><span v-text="item.commentList.length"></span></i>
								</div>
							</div>
							<div class="comment-list">
								...
								<div class="userly" v-show="item.commentList.length === 0 ? false : true">
									...
									<div class="more" @tap="goDetail(item, item.suggest.id)" v-if="item.commentList.length > 9">查看更多评论</div>
								</div>
							</div>
						</li>
					</ul>
		...
		<transition name="fade">
			<div class="preview-img" v-show="previewShow">
				...
				<div id="preview-box" :style="'width:' + (100 * previewList.length) + '%;'">
					<div class="preview-img-box" @tap="previewShow = false" 
